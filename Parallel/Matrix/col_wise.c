...
void init(){
	...
	for (i=0;i<all_block_size[pid];i++)
		A[i] = &Astorage[i*size];
	bi = (int*)malloc(all_block_size[pid]*sizeof(int));
	ci = (int*)calloc(size,sizeof(int));
	...
	if (pid==0){
		...
		for (i=0;i<size;i++) b[i] = rand()%5+1;
	}
	MPI_Scatterv(b,all_block_size,block_count,MPI_INT,bi,all_block_size[pid],MPI_INT,0,MPI_COMM_WORLD);
}
...
run(){
	...
	MPI_Reduce(ci,c,size,MPI_INT,MPI_SUM,0,MPI_COMM_WORLD);	
}

void print(){
	printf("-------[Processor %d] Row of Matrix A-------\n",pid);
	for (i=0;i<all_block_size[pid];i++){
		for (j=0;j<size;j++){
			printf("%d ",A[i][j]);
		}
